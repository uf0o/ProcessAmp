...
#include <shlwapi.h>
#include <memory>
#include "..\processamp\procampCommon.h"
...
int wmain(int argc, const wchar_t *argv[]) {
	if (argc < 4) {
		printf("Usage: procamp <unique_process_name> <priority> <refresh interval in seconds>\n");
		return 0; 
	}
	...
	int interval = _wtoi(argv[3])*1000;
	
	printf("[*] New threads will be polled every %d seconds\n", interval/1000);
	while(true) {

	int pid;
	int threads_list[1000];
	LPCWSTR processName = argv[1];
	pid = GetProcessPid(processName);
	int totalThreads = GetProcessThreads(pid,threads_list);
	
	ThreadData data;
	data.Priority = _wtoi(argv[2]);

	int i = 0;
	printf("Total Threads: %d\n", totalThreads);
	for (i = 0; i < totalThreads; i++) {
		//printf("ThreadID: %d\n", threads_list[i]);
		HANDLE hDevice = CreateFile(L"\\\\.\\processamp", GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
		...
		data.ThreadId =  threads_list[i];

		if (!DeviceIoControl(hDevice, IOCTL_PROCAMP_SET_PRIORITY, &data, sizeof(data), nullptr, 0, &bytes, nullptr)) {
			printf("Error setting priority (%u)\n", GetLastError());
			return 1;
		}
		CloseHandle(hDevice);
